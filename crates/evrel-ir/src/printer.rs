...
use crate::{
    ...
    ExceptionHandlerKind, FunctionId, FunctionIr, FunctionKind, FunctionMode, FunctionParameter,
    FunctionParameterKind, JsxAttribute, JsxAttributeName, JsxAttributeValue, JsxChild,
    JsxElementName, JsxMemberBase, LabeledStatementData, LabeledStatementId, MetaPropertyKind,
    ModuleIr, ObjectAssignmentProperty, ObjectBindingProperty, ObjectLiteralEntry,
    ObjectLiteralKey, ObjectMethodKind, OperationData, OperationKind, PrivateNameData,
    PrivateNameId, PropertyKey, RegionId, RegionOwner, SuperPropertyKey, TypeofTarget,
    UnaryOperator, UnwindTarget, UpdateOperator, ValueId, YieldKind,
};
...
pub fn print_function(function: &FunctionIr) -> String {
...
impl<'ir> ModulePrinter<'ir> {
    fn new(module: &'ir ModuleIr) -> Self {
        Self { module }
    }
    ...
    fn print(self) -> String {
        ...
        let exports = self
            ...
            .map(|export| match export {
                crate::ModuleExport::Local { exported, binding } => {
                    format!(
                        "  export @{} as {}",
                        binding.index(),
                ...
                crate::ModuleExport::Star { source, attributes } => {
                    let attributes = print_module_attributes(attributes);
    ...
    fn print_module_function(&self, id: FunctionId, function: &FunctionIr) -> String {
...
impl<'ir> FunctionPrinter<'ir> {
    fn new(function: &'ir FunctionIr) -> Self {
        Self {
            function,
            lines: Vec::new(),
        }
...
mod tests {
    use crate::{
        ...
        IsNullishOp, JsxElementName, JsxElementOp, LoadGlobalOp, ModuleBuilder, ModuleIr,
