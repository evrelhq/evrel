...
use crate::{
    ...
    ExceptionHandlerId, ExceptionHandlerKind, FunctionId, FunctionIr, FunctionKind, FunctionMode,
    FunctionParameterKind, FunctionProperties, LabeledStatementData, LabeledStatementId, ModuleIr,
    OperationId, OperationKind, OperationSuccessor, PrivateNameId, RegionId, RegionYieldOp,
    TemplateSiteId, UnwindTarget, ValueId,
};

/// Builds one function while tracking the current insertion block.
pub struct FunctionBuilder<'ir> {
    module: &'ir mut ModuleIr,
    function: FunctionId,
...
impl<'ir> FunctionBuilder<'ir> {
    ...
    pub(crate) fn new(module: &'ir mut ModuleIr, function: FunctionId) -> Self {
    ...
    pub fn append_operation(
        ...
        self.current_function_mut()
            .append_operation(block, unwind_target, kind, operands)
    }
    ...
    pub fn terminate(
        ...
        self.current_function_mut()
            .set_terminator(block, unwind_target, kind, operands)
    }
    ...
    fn current_function(&self) -> &FunctionIr {
        self.module
            .function(self.function)
            .expect("function must remain live")
    }

    fn current_function_mut(&mut self) -> &mut FunctionIr {
        self.module
...
mod tests {
    use crate::{
        ...
        FunctionParameterKind, JumpOp, LoopKind, ModuleBuilder, ModuleIr, OperationKind,
    ...
    fn builds_and_attaches_an_inline_expression_region() {
        ...
        let (region, owner) = {
            ...
            let value = builder.operation_results(constant)[0];
            builder.finish_region(region, [value], crate::UnwindTarget::Propagate);
