...
use crate::{BlockId, ConstantValue, JsFunctionIr, OperationId, ValueId};
...
mod tests {
    ...
    fn replaces_an_operation_with_a_constant_without_changing_its_result() {
        ...
        let (left, right, operation, result, return_operation, location) = {
            let mut module_builder = ModuleBuilder::new(&mut module);
            let file = module_builder.add_source_file("input.mjs", "20 + 22");
            let location = module_builder.source_location(file, crate::TextRange::new(0, 7));
            let mut builder = module_builder.function_builder(function_id);
        ...
        assert_eq!(operation_data.location(), location);
