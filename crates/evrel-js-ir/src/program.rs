...
use crate::{JsModuleIr, ModuleAttribute, ModuleId};
...
impl ProgramModule {
    ...
    pub const fn ir(&self) -> &JsModuleIr {
        &self.ir
    }
}
...
impl JsProgramIr {
    ...
    pub fn add_module(&mut self, key: ModuleKey, ir: JsModuleIr) -> ModuleId {
        assert!(
    ...
    pub fn module_ir_mut(&mut self, id: ModuleId) -> Option<&mut JsModuleIr> {
    ...
    fn remove_modules(&mut self, modules: &BTreeSet<ModuleId>) {
        for module in modules {
            assert!(
                self.modules.get(*module).is_some(),
                "a removed module must be owned by the program"
            );
            assert!(
                !self.entry_modules.contains(module),
                "an entry module cannot be removed"
            );
        }

        for dependency in &self.dependencies {
            if modules.contains(&dependency.importer()) {
                continue;
            }
            ...
            assert!(
                !matches!(
                    dependency.target(),
                    ModuleTarget::Internal(target) if modules.contains(target)
                ),
                "a live module cannot depend on a removed module"
            );
        }

        self.dependencies
            .retain(|dependency| !modules.contains(&dependency.importer()));
        self.modules_by_key
            .retain(|_, module| !modules.contains(module));

        for module in modules {
            self.modules
                .remove(*module)
                .expect("validated module must remain live");
        }
    }
}

impl Default for JsProgramIr {
    fn default() -> Self {
        Self::new()
    }
}

/// Mutates a whole program while preserving module-graph invariants.
pub struct ProgramEditor<'ir> {
    program: &'ir mut JsProgramIr,
}

impl<'ir> ProgramEditor<'ir> {
    /// Creates an editor for an existing program.
    pub fn new(program: &'ir mut JsProgramIr) -> Self {
        Self { program }
    }

    /// Atomically removes modules and their outgoing dependency edges.
    pub fn remove_modules(&mut self, modules: impl IntoIterator<Item = ModuleId>) {
        self.program
            .remove_modules(&modules.into_iter().collect::<BTreeSet<_>>());
    }
}
...
mod tests {
    ...
    fn owns_modules_entry_modules_and_dependencies() {
        ...
        let entry = program.add_module(entry_key.clone(), JsModuleIr::new());
        let dependency = program.add_module(dependency_key, JsModuleIr::new());
    ...
    fn rejects_duplicate_module_keys() {
        ...
        program.add_module(key.clone(), JsModuleIr::new());
        program.add_module(key, JsModuleIr::new());
    }
}
