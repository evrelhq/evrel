...
use evrel_js_ir::{BlockId, BlockParameterSource, JsFunctionIr, OperationKind, RegionId, ValueId};
...
impl JsExpressionRegionPlan {
    pub(crate) fn build(function: &JsFunctionIr, region: RegionId) -> Result<Self, JsCodegenError> {
        let data = function
            .region(region)
