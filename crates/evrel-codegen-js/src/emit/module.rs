...
use evrel_ir::{
    BindingId, DynamicImportOp, DynamicImportPhase as IrDynamicImportPhase, ModuleAttribute,
    ModuleExport, ModuleExportName as IrModuleExportName, ModuleImport, ModuleIr,
};
...
use crate::{JsCodegenError, plan::JsModulePlan};
...
pub(crate) fn attach_local_export_declarations<'ast>(
    ...
    for export in module.exports() {
        let ModuleExport::Local { exported, binding } = export else {
            continue;
        };
...
pub(crate) fn emit_module_exports<'ast>(
    ...
    for export in module.exports() {
        ...
        statements.push(match export {
            ...
            ModuleExport::Star { source, attributes } => Statement::new_export_all_declaration(
                SPAN,
                None,
                emit_string_literal(builder, source),
                emit_module_attributes(builder, attributes),
...
fn emit_binding_identifier<'ast>(
    builder: &AstBuilder<'ast>,
    plan: &crate::plan::JsFunctionPlan,
    binding: evrel_ir::BindingId,
