...
use evrel_ir::{
    BindingId, FunctionId, FunctionIr, ModuleIr, OperationId, OperationKind, RegionId,
    ValueDefinition, ValueId,
};

use crate::{
    JsCodegenError,
    name::{JsNameAllocator, JsReservedNames},
};
...
pub(crate) struct JsFunctionPlan {
    values: DenseMap<evrel_ir::ValueId, JsValueRepresentation>,
...
impl JsFunctionPlan {
        ...
        for (value_id, value) in function.values() {
            ...
            let representation = match operation_data.kind() {
                ...
                OperationKind::TaggedTemplate(template)
                    if matches!(
                        template.target(),
                        evrel_ir::CallTarget::Value {
                            receiver: evrel_ir::CallReceiver::Explicit
                        }
                    ) =>
                {
                    None
        ...
        for (operation_id, operation) in function.operations() {
            let operation_plan = match operation.kind() {
                ...
                OperationKind::InitializeBinding(initialize)
                    if module
                        .binding(initialize.binding())
                        .is_some_and(|binding| binding.kind() == evrel_ir::BindingKind::Var)
                        && operation
                            .operands()
                            .first()
    ...
    pub(crate) fn value(&self, value: evrel_ir::ValueId) -> Option<JsValueRepresentation> {
...
fn is_undefined(function: &FunctionIr, value: ValueId) -> bool {
        ...
        function.operation(operation).is_some_and(|operation| {
            matches!(
                ...
                OperationKind::Constant(constant)
                    if matches!(constant.value(), evrel_ir::ConstantValue::Undefined)
            )
        })
    })
}
...
fn is_hoisted_function_initialization(
    ...
    if !module
        .binding(binding)
        .is_some_and(|binding| binding.kind() == evrel_ir::BindingKind::Function)
    {
        return Ok(false);
    }
    let entry = function
...
fn is_direct_creation_use(function: &FunctionIr, definition: OperationId, value: ValueId) -> bool {
...
fn is_direct_eval_use(function: &FunctionIr, value: ValueId) -> bool {
    let Some(value) = function.value(value) else {
        return false;
    };
    ...
    matches!(
        ...
        OperationKind::Call(call)
            if matches!(
                call.target(),
                evrel_ir::CallTarget::Value {
                    receiver: evrel_ir::CallReceiver::None
                }
            )
    )
}
